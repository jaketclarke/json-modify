...
def test_dump_json():
    # set up output
    outputfile = f'output{os.sep}dir-one{os.sep}four.json'
    outputfolder = f'output{os.sep}dir-one'
    input = {"id":4,"name":"Patricia Lebsack","username":"Karianne","email":"Julianne.OConner@kory.org","address":{"street":"Hoeger Mall","suite":"Apt. 692","city":"South Elvis","zipcode":"53919-4257","geo":{"lat":"29.4572","lng":"-164.2990"}},"phone":"493-170-9623 x156","website":"kale.biz","company":{"name":"Robel-Corkery","catchPhrase":"Multi-tiered zero tolerance productivity","bs":"transition cutting-edge web services"}}
    # ensure dir exists
    make_directorytree_if_not_exists(outputfolder)
    # dump to disk
    dump_json(input,outputfile)
    # read from disk and check the result is the same
    result = load_json(outputfile)
